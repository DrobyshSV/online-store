...
class Basket extends ProductPage {
  ...
  async getBasketInfo() {
    ...
    mainWrapper.addEventListener('click', async (e) => {
      ...
      if (target.classList.contains('basket_button-plus') || target.classList.contains('basket_button-minus')) {
        const basketProductPrice = (target.parentElement as HTMLElement).nextElementSibling as HTMLElement;
        const pag = (
          (((target.parentElement as HTMLElement).parentElement as HTMLElement).parentElement as HTMLElement)
            .parentElement as HTMLElement
        ).parentElement as HTMLElement;
        let count = JSON.parse(localStorage.count);
        let price = JSON.parse(localStorage.price);
        const basketId = JSON.parse(localStorage.basketId);
        const arrayId = JSON.parse(localStorage.id);
  ...
  async promoValidation() {
    ...
    mainForm.onsubmit = (e) => {
      const promoVal: string = formInput.value;
  ...
  async formValidation() {
    ...
    headerLimitForm.onsubmit = (e) => {
      const limitVal: string = headerLimitInput.value;
